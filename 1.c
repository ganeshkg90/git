...
NODE deletefront(NODE first)
{
	...
	if(first->rlink ==NULL){
		printf("emp details ssn %d",first->ssn);
		free(first);
		return NULL;
	}
	temp = first->rlink ;
...
NODE deleterear(NODE first)
{
	...
	while(cur->rlink!=NULL)
		cur=cur->rlink ;
	temp = cur->llink ;
	printf("emp details ssn %d",cur->ssn);
	temp->rlink =NULL;
                free(cur);
                return first;
}
...
NODE dequeuesim(NODE first){
	int ch ;
	while(ch!=3)
	{
printf("dequeue operation using DLL");
printf("\n1.insert front\t 2.insert rear\t 3.delete front\n4.delete rear\t5.exit");
 printf("\nenter choice:");
 scanf("%d",&ch);
 switch(ch)
 {
...
void main()
{
 ...
 while(1)
 {
 printf("\n1.insert front\t 2.insert rear\t 3.delete front\n4.delete rear\t5.display\t 6.dequeue \t 7. exit	 ");
 printf("\nenter choice:");
 scanf("%d",&ch);
 switch(ch)
 {
